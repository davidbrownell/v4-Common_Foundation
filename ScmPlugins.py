...
from typing import ClassVar, Iterator, Optional, Pattern
...
class _CommitMessageDecorator(SCMPlugin):
    ...
    def Execute(
        ...
        if new_title != change.title or new_description != change.description:
            ...
            dm.WriteVerbose("No changes were made.")
        ...
        if not change.title:
            dm.WriteError(
                textwrap.dedent(
                    ...
                    The commit message for '{} <{}>' is empty.
...
class _ValidateBannedText(SCMPlugin):
    ...
    def ExecuteBatch(
        ...
        cached_statuses: dict[Path, str] = {}
        errors: dict[Path, str] = {}

        for change, change_dm in changes:
            filenames = list(
                itertools.chain(
            ...
            for filename_index, filename in enumerate(filenames):
                ...
                with change_dm.Nested(
                    ...
                    status = cached_statuses.get(filename, None)
                    if status is None:
                        if not filename.is_file():
                            ...
                            try:
                                ...
                                for match in self.__class__._banned_regex.finditer(filename_content):  # pylint: disable=protected-access
                                ...
                                if results:
                                    filename_dm.result = -1

                                    errors[filename] = TextwrapEx.Indent(
                                        textwrap.dedent(
                                            """\
                                            {}
                                            {}
                        ...
                        cached_statuses[filename] = status
        ...
        if errors:
            ...
            dm.WriteError(
                textwrap.dedent(
                    ...
                    "\n".join(errors[key] for key in keys).rstrip(),
                ),
            )
        else:
            ...
            if has_major and has_minor:
                dm.WriteError(
                    textwrap.dedent(
                        ...
                        Both major and minor AutoSemVer increments were specified in '{} <{}>'.
                ...
                dm.WriteError(
                    textwrap.dedent(
                        ...
                        The change '{} <{}>' increments a major or minor version but the files in the change are not associated with an AutoSemVer configuration file.
...
class _ValidateDistinctAutoSemVerConfigurations(SCMPlugin):
    ...
    def Execute(
        ...
        if dm.result == 0:
            dm.WriteVerbose("All files are distinct.")
...
class _ValidatePythonLibraryVersions(SCMPlugin):
    ...
    def ExecuteBatch(
        ...
        dm: DoneManager,
        repository: Repository,
        changes: Iterator[tuple[ChangeInfo, DoneManager]],  # pylint: disable=unused-argument
    ) -> None:
