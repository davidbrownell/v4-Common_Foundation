...
def commit_msg(
    ...
    with _YieldCommandLineDoneManager(verbose=verbose, debug=debug) as dm:
        ...
        with commit_message_file.open() as f:
            commit_message_file_content = f.read()

        original_commit_info = HookImpl.CommitInfo(
            "HEAD",
            "{} <{}>".format(name, email),
            commit_message_file_content,
            pfr.files_added,
            pfr.files_modified,
            pfr.files_removed,
        )

        commit_info = copy.deepcopy(original_commit_info)
        ...
        if commit_info != original_commit_info:
            if commit_info.files_added != original_commit_info.files_added:
                raise Exception("Changes to files added is not supported yet")
            if commit_info.files_modified != original_commit_info.files_modified:
                raise Exception("Changes to files modified is not supported yet")
            if commit_info.files_removed != original_commit_info.files_removed:
                raise Exception("Changes to files removed is not supported yet")

            new_commit_message_file_content = commit_message_file_content.replace(
                original_commit_info.description,
                commit_info.description,
            )

            with commit_message_file.open("w") as f:
                f.write(new_commit_message_file_content)
